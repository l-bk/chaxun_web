...
<body>
	...
	<form:form id="searchForm" modelAttribute="cxConsumableInfo" action="${ctx}/consumable/cxConsumableInfo/" method="post" class="breadcrumb form-search">
		...
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
			...
			&nbsp;<label class="control-label">适用机型：</label>
				<form:input path="conCompatible" htmlEscape="false"  class="input-xlarge "/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			...
			<th>打印页数和寿命</th>
			<th>参考价格</th>
			<shiro:hasPermission name="consumable:cxConsumableInfo:edit"><th>操作</th></shiro:hasPermission>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="cxConsumableInfo">
			<tr>
				...
				<shiro:hasPermission name="consumable:cxConsumableInfo:edit"><td>
    				<a href="${ctx}/consumable/cxConsumableInfo/form?id=${cxConsumableInfo.conId}">修改</a>
					<a href="${ctx}/consumable/cxConsumableInfo/delete?id=${cxConsumableInfo.conId}" onclick="return confirmx('确认要删除该耗材信息吗？', this.href)">删除</a>
